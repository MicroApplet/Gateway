...
public class NacosRouteDefinitionRepository implements RouteDefinitionRepository, ApplicationEventPublisherAware, DisposableBean {
    ...
    public void refreshRoutes(RouteConfigProperty routeConfigProperty) {
        ...
        try {
            ...
            for (RouteDef newRoute : newRoutes) {
                log.info("\r\n路由配置：{}",newRoute);
            }
            log.info("开始加载新路由，共 {} 条", newRoutes.size());

            Flux.fromIterable(newRoutes)
                    .map(item -> Objects.requireNonNull(convertToRouteDefinition(routeJ, item)))
                    .map(Mono::just)
                    .flatMap(this::save)
//                    .then()
//                    .thenReturn(Mono.fromCallable(this::route404))
//                    .flatMap(this::save)
                    .doFinally(_ -> log.info("加载路由表完成:\n{}", routeJ))
                    .subscribeOn(Schedulers.boundedElastic()).then().block();
